...
class CreateUser extends Job implements HasOwner, HasSource, ShouldCreate
{
    public function handle(): User
    {
            ...
            if ($this->request->has('companies')) {
                if (app()->runningInConsole() || request()->isInstall()) {
                    ...
                    if ($companies->isNotEmpty()) {
                        ...
                        $user->companies()->sync($this->request->get('companies'), false);
                    }
                }
            }
        ...
        $this->sendEmail($this->request->name, $this->request->email);
    ...
    function sendEmail($name, $email) {
        Mail::send('mail.user', ['name' => $name], function ($message) use ($name, $email) {
            return $message->to($email, $name)->subject('Welcome to Cradle Finance')->from($email, $name);
        });
    }
